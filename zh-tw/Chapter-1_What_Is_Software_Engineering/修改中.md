...
我們看到，程式設計和軟體工程之間有三個關鍵的區別：時間、規模和權衡取捨。在一個軟體工程專案中，工程師需要更重點考慮時間成本和需求變更。在軟體工程中，我們需要更加關注規模和效率，無論是對我們所生產的軟體，還是對生產軟體的組織。

最後，作為軟體工程師，我們被要求做出更複雜的決策，其結果風險更大，而且往往是基於對時間和規模增長的不確定性預估。

在 Google 內部，我們有時會說「軟體工程是隨著時間推移的程式設計」。程式設計當然是軟體工程的一個重要部分，畢竟，程式設計是產生新軟體的首要方式。如果您接受這個區別，那麼很明顯，我們可能需要在程式設計任務（開發）和軟體工程任務（開發、修改、維護）之間劃分。時間的增加為程式設計增加了一個重要的新維度。這就像從二維的正方形模型轉變為三維的立方體模型，距離不再只是速度的表示。因此，軟體工程並不僅僅是程式設計。

一種理解時間對程式碼影響的方式是思考「程式碼的預期生命週期[^1]是多少？」這個問題的合理答案大約相差 100,000 倍。想像一下生命週期只有幾分鐘的程式碼和將持續執行數十年的程式碼之間的差異是合理的。通常，週期短的程式碼不受時間影響。對於只需要存活一個小時的程式，您不太可能考慮到底層庫、作業系統（OS）、硬體或語言版本的新更新。這些短期系統實際上只是一個程式設計問題，就像在一個維度中壓縮得足夠扁平的立方體是正方形一樣。

隨著我們擴大時間維度，允許更長的生命週期，改變變得更加重要。在十年或更長的時間內，大多數程式的依賴關係，無論是隱含的還是顯式的，都可能發生變化。這一認識是我們區分軟體工程和程式設計的根本原因。
